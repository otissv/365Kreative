...
export async function deleteSubscribe(email: string) {
  try {
    if (!email) {
      throw new Error('Subscription not found')
    }

    const validate = subscribeEmailValidate(email)
    const error = (validate as ErrorIssues).errors
    ...
    const subscribe = await xata.db.subscribe.filter({ email }).getFirst()

    if (!subscribe) {
      throw new Error('Subscription not found')
    }

    const result = await subscribe.delete()
